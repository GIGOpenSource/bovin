/**
 * 浏览器内「面板原生 K 线」：参考移动端 KSChart 类 App 的分区布局（主图 + 成交量 + MACD），
 * 数据完全来自现有面板管道（uiState.dataCandles / flatten 后的 mini），不发起额外网络请求。
 */
...
export function drawPanelKlineCanvas(canvas, opts) {
  ...
  const dpr = typeof window !== "undefined" && window.devicePixelRatio ? Math.min(2.5, window.devicePixelRatio) : 1;
  ...
  ctx.fillRect(padL, volTop, innerW, volH);
  for (let i = 0; i < n; i++) {
    const r = viewRows[i];
    const o = Number(r[1]);
    const c = Number(r[4]);
    ...
    ctx.fillRect(cx - candleW / 2, volTop + volH - bh - 2, candleW, bh);
  }
  ctx.fillStyle = txtC;
